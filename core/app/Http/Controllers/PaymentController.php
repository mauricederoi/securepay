...
class PaymentController extends Controller
{
    ...
    public function userDataUpdate($data)
    {
        if ($data->status == 0) {
            ...
            $txt = 'Your Deposit of N' . number_format($data->amount) . ' via ' . $data->gateway->name . ' was successful ' . '.'. '\n \nAvail Bal: N' . number_format(round($user->balance)) . '' . '\nThank You for using Payonhold.' . ' ' ;
            $msg = 'We wish to inform you that your Deposit of N' . number_format($data->amount) . ' via ' . $data->gateway->name . ' was successful ' . '.'. '<br></b><br></b>Avail Bal: N' .  number_format(round($user->balance))  . '' . '<br></b><br></b>Thank You for using Payonhold.' . '<br></b><br></b>For Enquiries kindly send us an Email hello@payonhold.ng or Call +234-817-581-6285 and our friendly and well trained Customer Care Representative will be glad to help. ' ;
        ...
        try {
            ...
            try {
                ...
                if ($charge['status'] == 'succeeded') {
                    //Update User Data
                    $this->userDataUpdate($data);
                    return redirect()->route('deposit')->with('success', 'Success');
                }
            } catch (Exception $e) {
        ...
        if(verifychecksum_e($_POST, $paytm_merchant_key, $_POST['CHECKSUMHASH']) === "TRUE") {
            ...
            if($_POST['RESPCODE'] == "01"){
                ...
                if($responseParamList['STATUS'] == 'TXN_SUCCESS' && $responseParamList['TXNAMOUNT'] == $_POST['TXNAMOUNT']) {
                    ...
                    $t = 'alert';
                    $m = 'It seems there are some issue in server to server communication. Kindly the administrator';
                }
            } else {
                $t = 'alert';
                $m = $_POST['RESPMSG'];
                ...
                if (!$err)
                {
                    ...
                    if (!$err)
                    {
                        switch ($_POST['m_status'])
                        {
                            ...
                            default:
                                $message .= " - the payment status is not successful\n";
                                $err = true;
                                break;
                        }
                    }
                ...
                if($result['data']){
                    if ($result['data']['status'] == 'success') {
                        ...
                        if ($am == $sam) {
                            ...
                            return redirect()->route('deposit')->with('alert', 'Less Amount Paid. Please Contact tth Admin');
                        }
                    }else{
